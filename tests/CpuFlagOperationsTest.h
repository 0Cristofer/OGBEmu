...
class CpuFlagOperationsTest : public BaseTest
{
public:
    CpuFlagOperationsTest();
...
private:
    void TestFlagSetOperations();
    void TestFlagClearOperations();
    void TestArithmeticFlags();
    void TestCompareFlags();
    void TestLogicalFlags();
    void TestConditionalJumps();
    void TestConditionalCalls();
    void TestConditionalReturns();
    
    void WriteProgramToCartridge(const std::vector<byte>& program);
    void ExecuteProgram(int cycles);
    void VerifyFlagState(bool z, bool n, bool h, bool c, const std::string& testName);
