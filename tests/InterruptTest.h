...
class InterruptTest : public BaseTest
{
private:
    // Test components
...
private:
    // Test methods
    void TestInterruptEnableRegister();
    void TestInterruptFlagRegister();
    void TestVBlankInterrupt();
    void TestLcdInterrupt();
    void TestTimerInterrupt();
    void TestSerialInterrupt();
    void TestJoypadInterrupt();
    void TestInterruptPriority();
    void TestInterruptMasterEnable();
    void TestInterruptHandlerAddresses();
    
    // Helper methods
    void WriteProgramToCartridge(const std::vector<byte>& program);
    void TriggerInterrupt(byte interruptBit);
    void VerifyInterruptTriggered(word expectedPc, const std::string& interruptName);
